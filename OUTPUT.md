...
## PWA(プログレッシブウェブアプリ Progressive web apps)

PWA はウェブアプリをネイティブアプリのように扱えることを目的としたウェブアプリです

#### PWA である条件

- HTTPS で公開されているウェブサイトであること
- サービスワーカーが使用されていること(オフライン状態でも使える)
- マニフェストファイルが定義されていること(ブラウザ内ではなく個別のウィンドウで起動できる)

#### サービスワーカーとはなにか？

サービスワーカーはすでに学習した Web Worker の仲間で Worker の 1 つです。

#### サービスワーカーの機能

- プロキシ：ネットワークリクエストに介入しキャッシュ API を使ってキャッシュの制御などができる
- プッシュ通知：ブラウザに対してプッシュ通知ができる
- バックグラウンド同期：オフライン状態で処理をためておき、オンラインになった時点で処理を実行するための機能

「プッシュ通知」「バックグラウンド同期」はまだ全てのモダンブラウザに対応していません。

#### ウェブアプリマニフェスト

PWA のウェブ技術の一部で、ウェブアプリをインストールするために必要な情報(アプリの名前や説明文)を定義した JSON ファイルです。

> 端末のホーム画面にインストールした際に、ユーザーに素早いアクセスと豊かな操作性を提供するために必要なの情報を JSON テキストファイルで提供します。
